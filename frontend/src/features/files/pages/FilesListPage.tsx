  ...
  const columns = React.useMemo(
    () =>
      makeFileColumns({
        ...
        onUploadSig: (_row) => {
          /* будет в модалке на конкретной версии */
        },
    ...
    <div className="p-6">
      ...
      <UploadDialog
        ...
        requirements={
          <>
            <div>
              Допустимые расширения: <b>.pdf</b>
            </div>
            <div>
              Допустимый MIME: <code>application/pdf</code>
            </div>
            <div>
              Максимальный размер: <b>80 МБ</b>
            </div>
    ...
    <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center">
      <div className="bg-white rounded shadow-lg p-4 w-[560px]">
        ...
        <div className="mt-4 flex items-center justify-end gap-2">
          <button className="h-8 rounded-[var(--radius)] px-3 border" onClick={onClose}>
            Отмена
          </button>
          <button
            className="h-8 rounded-[var(--radius)] px-3 border text-white bg-black"
            onClick={() =>
              onSave({
                title,
    ...
    <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center">
      <div className="bg-white rounded shadow-lg p-4 w-[460px]">
        ...
        <div className="text-sm">
          Файл <b>{row.original_name}</b> будет помечен как удалённый и удалён из хранилища.
        </div>
        ...
        <div className="mt-4 flex items-center justify-end gap-2">
          ...
          <button className="h-8 rounded-[var(--radius)] px-3 border text-red-700" onClick={onConfirm}>
            Удалить
          </button>
        </div>
      </div>
          ...
          <div className="pt-2">
            ...
            <ul className="mt-1 space-y-1 text-sm">
              {pageItems.map((v, idx) => (
                <li
                  key={v.id}
                  className={`border rounded p-2 flex items-center gap-3 relative ${v.is_deleted ? "bg-red-50 border-red-300 " : v.is_latest ? "bg-amber-50 border-amber-300 " : ""}`}
                >
                  {/* Подсказка: иконка 'i' + выпадение вправо; смещение зависит от номера строки (1..5) */}
                  <RowTip indexInPage={idx + 1}>
                    <div>
                      <b>Версия:</b> #{v.id} {v.is_deleted && <b className="text-red-800">Версия удалена!</b>}
                    </div>
                    <div>
                      <b>Имя файла:</b> {v.original_name}
                    </div>
                    <div>
                      <b>Размер:</b> {(Number(v.size || 0) / 1024 / 1024).toFixed(2)} МБ ({v.size} байт)
                    </div>
                    <div>
                      <b>Загружено:</b> {new Date(v.created_at).toLocaleString()}
                    </div>
                    <div>
                      <b>MIME:</b> {v.mime || "—"}
                    </div>
                    <div>
                      <b>SHA-256:</b> <code className="break-all">{v.sha256 || "—"}</code>
                    </div>
                    <div>
                      <b>CRC32:</b> <code>{v.crc32 || "—"}</code>
                    </div>
                    {v.storage_path && (
                      <div>
                        <b>Хранилище:</b> <code className="break-all">{v.storage_path}</code>
                      </div>
                    )}
                  </RowTip>
                  {/* Номер версии */}
                  ...
                  {/* Имя файла */}
                  <span className="truncate w-[150px]" title={v.original_name}>
                    <b>{v.original_name}</b>
                  </span>
                  {/* Размер */}
                  ...
                  {/* CRC32 */}
                  <span className="text-xs w-[70px]">
                    CRC32: <code>{v.crc32 || "—"}</code>
                  </span>
                  {/* Действия справа */}
                  {!v.is_deleted && (
                    <div className="ml-auto items-center gap-2">
                      <button className="h-8 rounded-[var(--radius)] mr-2 px-2 text-xs border" onClick={() => onUploadSig(v.id)}>
                        Подпись
                      </button>
                      <button
                        className="h-8 rounded-[var(--radius)] px-2 text-xs border text-red-700 disabled:opacity-50"
                  ...
                  {v.is_deleted && <div className="ml-auto  items-center gap-2" />}
                </li>
              ))}
...
function RowTip({ children, indexInPage }: { children: React.ReactNode; indexInPage: number }) {
  const offsets = ["top-0", "top-10", "top-20", "top-32", "top-44"];
  const pos = offsets[indexInPage - 1] ?? "top-0";
  return (
    <div className="absolute left-0 -translate-x-full pr-2 w-0 group">
      <div className={`relative ${pos}`}>
        <div className="invisible group-hover:visible bg-zinc-900 text-white/90 text-xs rounded px-2 py-1 shadow-xl whitespace-pre-wrap max-w-xs">
          {children}
        </div>
      </div>
    </div>
