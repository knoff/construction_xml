    ...
    <div className="p-6">
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <button
              className="h-9 rounded-[var(--radius)] px-3 text-sm text-white bg-black"
              onClick={async () => {
      ...
      >
        <DialogContent>
          ...
          <DialogBody>
            <input
              defaultValue={data.find((d) => d.id === editId)?.name ?? ""}
              id="edit-name"
              className="w-full h-9 rounded-[var(--radius)] border px-3 text-sm"
            />
          </DialogBody>
          <DialogFooter>
            <DialogClose asChild>
            ...
            <button
              ...
              onClick={async () => {
                ...
                const r = await fetch(`/api/objects/${editId}`, {
                  method: "PATCH",
                  headers: { "Content-Type": "application/json" },
      ...
      >
        <DialogContent>
          ...
          <DialogBody>
            <div>
              Действие необратимо. Будет удалён объект {confirmName ? (
                <>
                  «<b>{confirmName}</b>»
                </>
              ) : (
                <>
                  ID: <b>{String(confirmId ?? "")}</b>
                </>
              )}.
            </div>
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                className="h-4 w-4"
                checked={confirmDeleteDocs}
                onChange={(e) => setConfirmDeleteDocs(e.target.checked)}
              />
              Также удалить связанные документы
              {confirmDocsCount ? (
                <>
                  ( <b>{confirmDocsCount}</b> )
                </>
              ) : null}
            </label>
          </DialogBody>
